...
- (void)showImage:(id)sender atIndexPath:(NSIndexPath *)indexPath
{
    if ([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPad) {
        Item * item = [[ItemStore sharedStore] allItems][0];
        ...
        CGRect rect = [self.view convertRect: self.view.bounds fromView: sender];
...
- (void)changeValue: (UIStepper *)sender atIndexPath:(NSIndexPath *)indexPath
{
    Item *item = [[ItemStore sharedStore] allItems][indexPath.row];
    item.valueInDollars = sender.value;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    StepperCell *cell = [tableView dequeueReusableCellWithIdentifier: NSStringFromClass([ItemCell class])];
    ...
    cell.stepper.value = [item valueInDollars];
...
- (void)viewDidLoad
{
    ...
    UINib *nib = [UINib nibWithNibName: @"StepperCell" bundle: nil];
